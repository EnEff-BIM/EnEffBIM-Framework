    ...
    for bldg_count in static_data.buildings:
            ...
            if comp_count.map_control != None:
                if comp_count.map_control.control_strategy == "T_set_const" or \
                   comp_count.map_control.control_strategy == "dp_const":
                    ...
                    sys = static_data.create_system("Modelica.Blocks.Sources.Constant", comp_count.map_control.control_strategy, [con])
        ...
        for zone_count in bldg_count.thermal_zones:
            for comp_zone_count in zone_count.supply_components:
                ...
                if comp_zone_count.map_control != None:
                    if comp_zone_count.map_control.control_strategy == "room_T_PID1":
                    
                         con = static_data.create_connector(None, "y", "Real")
                         sys = static_data.create_system("Modelica.Blocks.Sources.Constant", (comp_zone_count.map_control.control_strategy+"_T_set"), [con])
                         for para in comp_zone_count.map_control.parameters:
                             static_data.create_mapped_property(sys, para.name, para.value)
                         con.parent = sys
                        
                         con_a = static_data.create_connector(None, "T", "Real")
                         con_b = static_data.create_connector(None, "port", "Heat")
                         sys1 = static_data.create_system("Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor", (comp_zone_count.map_control.control_strategy+"_sens"), [con])
                         con_a.parent = sys1
                         con_b.parent = sys1
                         ...
                         sys2 = static_data.create_system("Modelica.Blocks.Continuous.LimPID", (comp_zone_count.map_control.control_strategy+"_contrl"), [con])
                         for para in comp_zone_count.map_control.parameters:
                         ...
                         static_data.add_connection(con_e, comp_zone_count.map_control.control_connector)
